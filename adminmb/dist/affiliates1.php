...
require __DIR__ . '/shopify/config.php';
session_start();

$me = strtolower(trim((string)($_SESSION['admin_email'] ?? $_SESSION['user_email'] ?? '')));
if ($me === '') {
    header('Location: /shopify/login.php');
    exit;
}
    ...
    if ($do === 'delete' && $code !== '') {
        ...
        if ($r = $q->fetch(PDO::FETCH_ASSOC)) {
            foreach ([$r['profile_img'], $r['id_img']] as $f) {
                if (!$f) continue;
                $abs = realpath(__DIR__ . '/shopify/uploads/affiliates/' . basename($f));
                $root = realpath(__DIR__ . '/shopify/uploads/affiliates');
...
.sub{font-size:12px;color:var(--mute);margin-bottom:18px}
