/* Video Upload — admin tab (layout pass, no backend yet)
   Renders the upload form + bucket browser into #vidupPerfRoot.
...
(function(){
  ...
  window.vidupPerfLoad = function(){
    var root = document.getElementById('vidupPerfRoot');
    if (!root) return;
        ...
        + '<div>'
          + '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;gap:14px;flex-wrap:wrap">'
            ...
            + '<div style="display:flex;gap:10px;align-items:center">'
              + '<input id="vidupSearch" placeholder="Search by filename, stage, or email…" oninput="vidupSearch(this.value)" style="background:#0e1117;border:1px solid #2a2f3e;border-radius:8px;padding:7px 11px;color:#e8eaf0;font-size:12px;width:100%;max-width:300px;box-sizing:border-box"/>'
              + '<button onclick="vidupPerfLoad()" style="padding:7px 14px;background:transparent;border:1px solid #2a2f3e;color:#8a8fa8;border-radius:7px;font-size:12px;cursor:pointer">Refresh</button>'
            + '</div>'
          + '</div>'
